...
def main():
    ...
    data = data.fillna(-100)    #oppure prendere un numero a caso fuori schermo

    X_train = data.iloc[:,2:].values    #colonne dei dati (posizione oggetto e dati lidar)
    y_train = data.iloc[:,1].values    #colonna 1 è il comando eseguito

    algorithm = RandomForestClassifier(n_estimators=100, max_depth=5, random_state=0)
    algorithm.fit(X_train, y_train)

    print("Training score: %f" % np.mean(cross_val_score(algorithm, X_train, y_train, cv=5)))
    #create a list of -1
    ...
    for measurment in lidar.iter_measurments(max_buf_meas = config.MAX_BUF_MEAS):
        ...
        if isFull(measurments_list):
            ...
            dataList = [trackedObjectPosX, trackedObjectPosY] + measurments_list
            X = np.array(dataList)
            y = algorithm.predict(X) #per eseguire sulla base delle X
            #dopo traduzione y in comando
            commandSent(y)
