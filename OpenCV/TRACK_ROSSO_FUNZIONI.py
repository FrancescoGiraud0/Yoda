...
sensitivity = config.sensitivity                              #impostazioni colore
...
def Position(conts):
    if len(conts) > 0:
        x,y,w,h=cv2.boundingRect(conts[0])                       #funzione che calcola il rettangolo per il contorno, restituisce x,y,base e altezza
        cv2.rectangle(frame,(x,y),(x+w,y+h),(0,0,255), 2)        #disegno rettangolo
        cv2.putText(frame, str(0+1),(x,y+h),font,2.,(0,255,255)) #numerazione rettangolo 
        xCentroRett=x+w/2
        yCentroRett=y+h/2
...
while(1):
    ...
    Draw_line(frame,conts)   #disegno linee e contorno oggetti rossi individuati

    Position(conts)  #controllo posizione del centro dell'oggetto
